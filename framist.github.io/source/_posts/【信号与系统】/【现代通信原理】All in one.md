...
- [【现代通信原理】8 蜂窝系统](#现代通信原理8-蜂窝系统)
  ...
  - [参考](#参考-4)
    - [作业](#作业-5)
- [【现代通信原理】9 5G 移动通信系统与技术](#现代通信原理9-5g-移动通信系统与技术)
...
**一个信息序列，知道其符号是 128 元的，符号速率是 8000 波特，求该消息序列的比特速率。**
...
> QwQ：
> 
> 根据题目条件，已知调频波的振幅为 10V，瞬时频率为 \( f(t) = 10^6 + 10^4 \cos(2\pi \times 10^3 t) \) Hz。要求其表达式，步骤如下：
> 
> **步骤 1：确定瞬时角频率**
> 瞬时角频率 \( \omega(t) \) 是瞬时频率 \( f(t) \) 的两倍：
> \[
> \omega(t) = 2\pi f(t) = 2\pi \left[10^6 + 10^4 \cos(2\pi \times 10^3 t)\right] \, \text{rad/s}
> \]
> 
> **步骤 2：计算相位函数 \( \phi(t) \)**
> 相位 \( \phi(t) \) 是角频率对时间的积分：
> \[
> \phi(t) = \int_0^t \omega(\tau) \, d\tau + \phi_0
> \]
> 假设初始相位 \( \phi_0 = 0 \)，则：
> \[
> \phi(t) = \int_0^t \left[2\pi \times 10^6 + 2\pi \times 10^4 \cos(2\pi \times 10^3 \tau)\right] d\tau
> \]
> 
> **分步积分：**
> 1. **载波部分积分：**
> \[
> \int_0^t 2\pi \times 10^6 \, d\tau = 2\pi \times 10^6 \cdot t
> \]
> 
> 1. **调制部分积分：**
> 令 \( \theta = 2\pi \times 10^3 \tau \)，则 \( d\theta = 2\pi \times 10^3 \, d\tau \)，积分变为：
> \[
> \int_0^t 2\pi \times 10^4 \cos(2\pi \times 10^3 \tau) \, d\tau = 10 \cdot \sin(2\pi \times 10^3 t)
> \]
> 
> 
> **合并结果：**
> \[
> \phi(t) = 2\pi \times 10^6 \cdot t + 10 \cdot \sin(2\pi \times 10^3 t)
> \]
> 
> **步骤 3：写出调频波表达式**
> 调频波的一般形式为：
> \[
> u(t) = U_c \cos\left[\omega_c t + \phi_m \sin(\Omega t)\right]
> \]
> 其中：
> - \( U_c = 10 \, \text{V} \)（振幅）
> - \( \omega_c = 2\pi \times 10^6 \, \text{rad/s} \)（中心角频率）
> - \( \phi_m = 10 \)（最大相偏，即调频系数 \( m_f \)）
> - \( \Omega = 2\pi \times 10^3 \, \text{rad/s} \)（调制信号角频率）
> 
> 代入参数得：
> \[
> u(t) = 10 \cos\left[2\pi \times 10^6 t + 10 \sin(2\pi \times 10^3 t)\right]
> \]
> 
> **最终答案**
> 调频波的表达式为：
> \[
> \boxed{u(t) = 10 \cos\left(2\pi \times 10^6 t + 10 \sin(2\pi \times 10^3 t)\right)}
> \]
...
> Deepseek R1:
> 
> 已知某单频调频波的振幅是 10V，瞬时频率为$f(t)=10^6 + 10^4 \cos(2\pi \times 10^3 t) \
> \text{Hz}$，
> 
> （1）此调频波的表达式：
> 
> 调频波的表达式可以通过对瞬时频率积分得到相位，然后代入振幅得到。瞬时频率$f(t)$的积分结果如下：
> 
> $$ \theta(t) = 2\pi \int_0^t f(t') \, dt' = 2\pi \int_0^t \left(10^6 +
> 10^4 \cos(2\pi \times 10^3 t')\right) \, dt' $$
> 
> 积分分为两部分：
> 
> $$ \int_0^t 10^6 \, dt' = 10^6 t $$
> 
> $$ \int_0^t 10^4 \cos(2\pi \times 10^3 t') \, dt' = \frac{10^4}{2\pi
> \times 10^3} \sin(2\pi \times 10^3 t) = \frac{10}{2\pi} \sin(2\pi
> \times 10^3 t) $$
> 
> 因此，相位为：
> 
> $$ \theta(t) = 2\pi \times 10^6 t + 10 \sin(2\pi \times 10^3 t) $$
> 
> 调频波的表达式为：
> 
> $$ s(t) = 10 \cos\left(2\pi \times 10^6 t + 10 \sin(2\pi \times 10^3
> t)\right) $$
> 
> （2）此调频波的最大频率偏移、调频指数和频带宽度：
> 
> - 最大频率偏移$\Delta f$：瞬时频率相对于载波频率的最大偏移量为$10^4 \ \text{Hz}$，即$\Delta f = 10 \ \text{kHz}$。
> - 调频指数$\beta$：最大频率偏移与调制信号频率的比值，$\beta = \frac{\Delta f}{f_m} = \frac{10^4}{10^3} = 10$。
> - 频带宽度：根据卡森公式，频带宽度为$2(\Delta f + f_m) = 2(10^4 + 10^3) = 22 \ \text{kHz}$。
> 
> 最终答案：
> 
> （1）此调频波的表达式 $\boxed{10 \cos\left(2\pi \times 10^6 t + 10 \sin 2\pi
> \times 10^3 t\right)}$；
> 
> （2）最大频率偏移为$\boxed{10\ \text{kHz}}$，调频指数为$\boxed{10}$，频带宽度为$\boxed{22\
> \text{kHz}}$。
...
为了解决 2PSK 的问题，讨论差分相移键控，以未调载波相位为参考基准，绝对相移。由于倒$\pi$问题，解调可能出现反向工作，2DPSK，相对相移键控。对于 A 方式，解决了载波相位不确定问题，但对长码元仍存在码元定时问题。改用 B 方式，相邻码元间必有相位突跳。后者应用广泛。
...
![2DPSK 解调](https://pic4.zhimg.com/80/v2-53cd7b31c83e345c9873d866d2766aaf.png)
...
发送端输入的二进制数字基带信号需要先经过电平变换器转换为 M 电平的基带脉冲，然后再去调制。
...
MFSK 信号占用较宽的频带，信道频带利用率不高。

$$
B=\left| f_M-f_1 \right|+\frac{2}{T_B}
$$

MFSK 一般用于调制速率 $(1/T_B)$ 不高的衰落信道传输场合。

#### 调制解调

![MFSK 调制解调](https://pic4.zhimg.com/80/v2-8d8e1408a34553ebc1d90cc8b535af9a.png)

<br />

### MPSK 多进制相移键控

利用载波的 M 种不同**相位**表示数字信息。
...
> 多进制 PSK 不能采用一个相干载波进行解调，存在相位模糊问题，$cos\theta_k=cos(2\pi-\theta_k)$
>
> 令振幅$A=1, s_k(t)=cos(\omega_0*t+\theta_k)=a_k*cos(\omega_0*t)+b_k*sin(\omega_0*t)$,可以看做是正弦和余弦两个正交分量合成。$a_k^2+b_k^2=1$。MPSK 信号可以看做是两个 MASK 信号码元之和，带宽与 MASK 相同。
>
...
*波形和相位不连续、波形连续相位不连续，相位不连续时，最大相位跳变 180°，使限带的 QPSK 信号包络起伏很大，并出现包络零点，有时将码元周期包含整个载波周期的假设隐含不提，认为 PSK 信号初始相位相同，则码元边界瞬时相位一定连续*
...
* 最大相位跳变 180°，使**限带**的 QPSK 信号包络起伏很大，并出现包络零点 -> 出现旁瓣
* 频谱扩展大，旁瓣对邻道干扰大 -> 引起 ISI
...
**原理**：分解为两路 2PSK 信号的相干解调。

![Image](https://pic4.zhimg.com/80/v2-f6d5ea7130bded633f74b897dc84ca22.png)

*两路正交信号叠加，用两路相干载波去解调*

**存在问题**：存在 900 的相位模糊（0, 90, 180, 270）

**解决方案**：采用四相相对相位调制，即 Q**D**PSK。
...
QDPSK 与 QPSK 的关系，如同 2DPSK 与 2PSK 关系

QDPSK 的矢量图与 QPSK 的矢量图相似——只是参考相位是前一码元的载波相位

#### 调制解调

也有**正交调相法** 和 **相位选择法**

仅需在 QPSK 调制器基础上增添**差分编码（码变换）**
...
* 成形脉冲——构建 RC 谱特性，实现无 ISI
* 匹配滤波——SRC 配对，达到最大 SNR 解调，实现最佳传输（基带信号的最佳接收）
...
FSK——带宽由 M 子带构成，随 M 增加，频带利用率随 M 下降
...
**某系统采用 MPSK 调制方式，在加性白高斯噪声信道上以 50kb/s 的速率传输数据。假设系统频域传输特性是滚降系数等于 1 的升余弦函数，系统带宽为 50kHz。求：**
...
**某 2FSK 调制系统的码元速率为 2000Baud，已调信号的载频分别为 6000Hz（对应“1”码）和 4000Hz（对应“0”码）。求：**

（1）若发送的信息序列为 1011001，试画出 2FSK 信号的时域波形。
...
与 PCM 相比可减少量化器的量化级数，即编码位数减少，比特率降低，带宽减小
...
ADPCM 是为了改善 DPCM 的性能，而将**自适应技术**引入到量化
...
—— 可看作一种最简单的 DPCM（量化电平数取 2 即对预测误差进行 1 位编码）
...
* [X] 区群的复制；系统容量的确定。考察大题：系统容量 c，确定系统容量；基于区群尺寸 N，计算总信道数。
* [X] 区群尺寸与系统容量的关系，基于同频干扰
* [X] 小区规划。确定最近的同信道相邻小区的规则：利用 i, j 与时针旋转
* [X] 同道、邻道干扰
...
* 覆盖范围：30-50km，发射功率 50-200W，天线很高（>30m）
* 网络结构简单，频道数目少，无需无线交换，直接与 PSTN 连接
...
切换是指将当前正在通信的移动台的通信链路从当前基站转 移到另一基站的过程。（也称为 ALT——自动链路转换）
...
2. 顺（逆）时针方向旋转 60°，再数$j$个小区。则该小区为当前小区的一个相邻同频道小区；
3. 每个小区有 6 个相邻同频道小区。
...
从**提高容量**的角度出发，我们希望 N 越小（q 也越小，即每个小区分配的信道数更多）越好；

从**减小干扰**的角度出发，我们希望 N 越大（q 也越大，即同频小区分布越稀疏）越好。
...
假设每个小区都按半径的一半来 分裂，如图所示。为了用这些更小的小区来覆盖整个服务区域，将需要大约为原来小区数目 4 倍的小区。
...
![120 度裂向如何减小同频小区干扰](https://pic4.zhimg.com/80/v2-538506dc0db245e0d68f1b7195368176.png)
...
* 全向天线：6 个干扰源；
* 120 度定向天线：2 个或 3 个干扰源；
* 60 度定向天线：1 个干扰源

**扇区划分提高系统容量**

* 扇区划分提高系统容量的方法是保持小区半径 R 不变，而寻找办法来减小 D/R 比值。
* 在扇区划分的方法中，容量的提高是通过降 低同频干扰，从而减小区群中的小区数量 N，相应提高频率复用来实现的。
...
基于 7 小区复用的微小区概念，如图所示。在这个方案中，每 3 个 (或者更多) 区域站点 (在图中以 Tx/Rx 表示) 与 一个单独的基站相连，并且共享同样的无线设备。各微小区用同轴电缆、光导纤维或微波链路与基站连接。多 个微小区和一个基站组成一个小区。

![微小区的概念图](https://pic4.zhimg.com/80/v2-1360b26dc379fb9e1a19e8ecfe686275.png)

**新微小区优点**

* 当移动台在小区内从一个微小区运动到另一个微小区时，它使用同样的信道。移动台在小区内的微小区之间运动时**不需要 MSC 进行切换**。
...
**例 1：一个蜂窝系统中有 1001 个用于处理通信业务的可用信道，设小区的面积为 6km2，整个系统的面积为 2100km2。**

1. N=7，计算系统容量；
2. N=4，覆盖整个区域，需要将该区群覆盖多少次？计算系统容量；
3. 减小区群大小 N 能增大系统容量吗？解释原因。

![Image](https://pic4.zhimg.com/80/v2-e45ad8699b4808112fc27937afbc88d5.png)


**例 2**：某蜂窝系统的总带宽为 30MHz，采用两个 25kHz 的信道 组成全双工话音和控制信道，系统采用 9 小区复用方式，并将 总带宽中的 1MHz 分给控制信道。
...
**例 3**：在某系统中，接收机要求的信干比门限为 $ S/I >= 18dB$， $κ＝4$，求最小区群尺寸应为多少？
