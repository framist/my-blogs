...
**(不要求)**如第 4.7.2 节所述，Linux 不区分进程和线程。取而代之的是，Linux 以相同的方式对待两者，从而使任务更类似于进程或线程，具体取决于传递给`clone()`系统调用的标志集。但是，其他操作系统（例如 Windows）对进程和线程的处理方式也有所不同。通常，此类系统使用一种表示法，其中进程的数据结构包含指向属于该进程的单独线程的指针。
