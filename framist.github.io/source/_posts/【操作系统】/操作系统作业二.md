---
title: 【操作系统概念-作业2】Operating-System Structures
categories:
- 计算机科学 
- 课业学习
- 操作系统
...
> b. I/O操作。磁盘、磁带、串行线和其他设备必须在一个非常低的水平上进行通信。用户只需要指定设备和要对其进行的操作，而系统则将该请求转换为特定于设备或控制器的命令。不能相信用户级程序只访问他们应该访问的设备，并且只在其他情况下不使用这些设备时才访问。
